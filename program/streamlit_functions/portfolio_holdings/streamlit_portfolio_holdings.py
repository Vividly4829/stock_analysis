...
def streamlit_portfolio_holdings():
    ...
    dataframe_tab, sunburst_tab, historical_performance,  = st.tabs(
        ["Dataframe", "Sunburst", "Historical performance"])
    ...
    for category in categories:
        ...
        fig.add_trace(go.Bar(
            ...
            text=filtered_data['Percentage'].apply(lambda x: f'{x:.2f}%'),  # Show percentage with 2 decimal places
            textposition='inside',
            name=category
        ))
    ...
    wide_format_data = category_totals_per_day.pivot(index='Date', columns='Category', values='Value (NOK)').fillna(0)
    ...
    for category in wide_format_data.columns[1:]:  # Skip the 'Date' column
        fig.add_trace(go.Scatter(
            ...
            y=wide_format_data[category],  # Cumulative sum for stacked area plot
            mode='lines',
            name=category,
