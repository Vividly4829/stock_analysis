...
from program.workers.find_portfolio_valuation import calculate_portfolio_value, calculate_portfolio_total_value
...
class JsonBaseUserPortfolio:
    ...
    def create_new_portfolio(self):
        try:
            file_path = os.path.join(
                data_folder, self.user_name, f'{self.user_portfolio_name}.json')
            data = {
                'holdings': [{
                    "Ticker": "Example",
                    "Type": "Example",
    ...
    def get_portfolio_holdings_df(self, tickers: list | None = None):
        ...
        if self.user_portfolio is not None:
            try:
                ...
                df, exchange_rates = calculate_portfolio_value(
                    df, tickers=tickers)
    ...
    def load_inception_dates(self):
        if self.holdings is not None:
            self.holdings['Inception Date'] = self.holdings['Ticker'].apply(
                lambda x: self.get_etf_inception_date_yfinance(x))
    ...
    def get_portfolio(self):
        try:
            ...
            print('failed to get portfolio, ',
                  self.user_name, self.user_portfolio_name)
            return None
    ...
    def save_portfolio(self):
        try:
            ...
            portfolio_logs_folder = os.path.join(
                data_folder, self.user_name, 'portfolioLogs', self.user_portfolio_name)
    ...
    def update_portfolio_holdings(self):
        if self.holdings is not None and self.user_portfolio is not None:
            self.user_portfolio['holdings'] = self.holdings.to_dict(
                orient='records')
